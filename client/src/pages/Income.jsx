...
const EMPTY_FORM = { amount: "", source: "Salary", description: "", date: "" };
...
function PageHeader({ onAdd }) {
  return (
    ...
    >
      ...
      <Button
        onClick={onAdd}
        style={{ paddingInline: 16, background: "#22c55e", color: "#fff" }}
      >
...
function SummaryBar({ data, width }) {
  ...
  const cards = [
    ...
    },
    {
      ...
      color: SOURCE_COLORS[topSource?.[0]] || "#22c55e",
    },
    {
...
// ── IncomeTable ───────────────────────────────────────
function IncomeTable({ data, onEdit, onDelete, deleting, width }) {
  if (!data.length) {
    return (
      <div
        style={{
      ...
      >
        ...
        <p style={{ fontSize: 12 }}>
          Add your first income entry or adjust filters
        </p>
      </div>
    );
  }
  ...
  if (width < 768) {
    return (
      ...
      >
        {data.map((inc, i) => {
          const srcColor = SOURCE_COLORS[inc.source] || "#9ca3af";
          return (
            <div
              key={inc._id}
              style={{
              ...
              >
                ...
                <button
                  ...
                  style={{
                    ...
                    alignItems: "center",
                    justifyContent: "center",
                    cursor: deleting === inc._id ? "not-allowed" : "pointer",
                  }}
                >
                  {deleting === inc._id ? (
                    <Spinner size={12} />
  ...
  return (
    ...
    >
      ...
      >
        {["Description", "Source", "Date", "Amount", ""].map((h) => (
          <span
            key={h}
            style={{
              fontSize: 11.5,
...
// ── IncomeForm ────────────────────────────────────────
// inputStyle defined at module level — avoids "cannot create components during render"
    ...
    <form onSubmit={onSubmit} noValidate>
      ...
      <div style={{ marginBottom: 14 }}>
        ...
        <select
          ...
          style={{ ...inputStyle(errors.source), cursor: "pointer" }}
          onFocus={(e) => {
            e.target.style.borderColor = "#22c55e";
          }}
          onBlur={(e) => {
            e.target.style.borderColor = errors.source
              ? "#ef4444"
              : "var(--border)";
          }}
        >
...
export default function Income() {
  ...
  }, [raw, search, source, dateFrom, dateTo]);
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (modal === "edit") {
        await incomeAPI.update(editing._id, payload);
